...
object FlappyBox {
  ...
  def main(canvas: html.Canvas): Unit = {
    ...
    var dead = 0
    // What moveDistance this is; used to keep track
    ...
    // queue and in the current moveDistance.
    ...
    def runLive() = {
      ...
      if (moveDistance >= 0 && moveDistance % obstacleGap == 0)
        obstacles.enqueue(Random.nextInt(canvas.height - 2 * holeSize) + holeSize)
      if (obstacles.length > 7){
        obstacles.dequeue()
        // once you dequeue, there is a sudden shift, so you have to adjust the moving distance
        // this means moveDistance <= 1200, always
        moveDistance -= obstacleGap
      }
      ...
      for((holeY, i) <- obstacles.zipWithIndex){
        // Where each obstacle appears depends on what moveDistance it is.
        // This is what keeps the obstacles moving to the left as time passes.
        val holeX = i * obstacleGap - moveDistance + canvas.width
